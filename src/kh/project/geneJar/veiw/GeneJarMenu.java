...
public class GeneJarMenu {
	...
	public void join() { ////////// 회원가입 (고객용)
		...
		while (true) {
			...
			CustomerLogin cl = new CustomerLogin(id, password, name);
			if (new LoginController().join(cl)) {
	...
	public String emplogIn() { /////////////////// 로그인
		...
		while (true) {
			...
			if (new LoginController().adminlogIn(id, password) != null) {
				System.out.println(new LoginController().adminlogIn(id, password).getName() + "님, 환영합니다!");
				return new LoginController().adminlogIn(id, password).getDept();
			} else {
				System.out.println("틀린 아이디 또는 비밀번호입니다. 다시 입력해주세요.");
			}
		}
	...
	public void changePassword() { //////////// 비번 변경
		...
		while (true) {
			...
			if (new LoginController().changePassword(id, oldPW, newPW)) {
	...
	public void empChangePassword() { //////////// 비번 변경
		...
		while (true) {
			...
			if (new LoginController().adminChangePassword(id, oldPW, newPW)) {
				System.out.println("비밀번호 변경에 성공했습니다.");
				return;
			} else {
				System.out.println("비밀번호 변경에 실패했습니다. 다시 입력해주세요.");
	...
	public void insertEmp() {
		...
		if (yn == 'y' || yn == 'Y') {
			...
			if (new EmployeeController()
					.join(new Employee(empNo, id, password, name, gender, phone, dept, salary, bonus))) {
	...
	public void updateEmp() {
		...
		System.out.println("3. 사원 부서");
		System.out.println("4. 사원 연봉");
		System.out.println("5. 보너스 율");
		System.out.println("9. 돌아가기");
		System.out.print("메뉴 번호를 누르세요 : ");
		int menuNum = sc.nextInt();
		sc.nextLine();
...
//	---------------------------------------------------------------------------------약품관리
	...
	public void addMedicine() {
		...
		while (true) {
			...
			if (mc.addMedicine(new Medicine(name, med, formulation, target, medNo, prodCost, inven))) {
	...
	public void medOrder() {
		...
		while (true) {
			...
			if (oc.getOrder(orderNo) == null) {
				...
				fnc.changeE(quantity*mc.getMedicine(med).getProdCost());
